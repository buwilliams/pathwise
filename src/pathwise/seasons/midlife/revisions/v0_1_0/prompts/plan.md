...
**If the recommendation involves training, school, or a credential**, name a specific program from the research bundle that fits **{{ answers.interests | default("their stated interests") }}**, and say in one sentence why it fits. **If the recommendation involves starting a business**, ground it in the `business_landscape` numbers and {{ answers.business_idea | default("the idea they shared") }}. **If the recommendation involves a move**, ground it in the `relocation` numbers for {{ answers.move_destination | default("the destination they named") }}.
